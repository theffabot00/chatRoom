...
class DEBUG{
    ...
    Log(astr) {
        ...
        var dateStr = "[" + d.getHours() + ":" + d.getMinutes() + ":" + dS + "] "
        fs.appendFile(output, dateStr + astr + "\n",function(err) {
...
http.createServer(function(req, res){
    ...
    if (reqName != "getMessage") {
        var d = new Date();
        if (reqName == "") {
            Debug.Log( req.connection.remoteAddress + " pinged for nothing");
        } else {
            Debug.Log( req.connection.remoteAddress + " pinged for " + reqName);
        }
    }
